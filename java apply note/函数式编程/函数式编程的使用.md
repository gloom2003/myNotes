...
~~~java
    public ResultVO<List<Map<String, Object>>> getInvoicesByFolderId(Long folderId) {
        // 根据不同发票类型返回不同的发票信息进行展示
        List<Map<String, Object>> resultMapList = children.stream()
                .map(this::covertToResultMap)
                .collect(Collectors.toList());

        // 检查是否存在未设置 sortNumber 的发票
        boolean hasMissingSortNumber = resultMapList.stream().anyMatch(e -> e.get("sortNumber") == null);
        // 如果数据中有一个发票没有sortNumber，则按照默认排序后设置sortNumber
        if (hasMissingSortNumber) {
            AtomicLong counter = new AtomicLong(0);
            List<Map<String, Object>> sortedList = resultMapList.stream()
                    .sorted(Comparator.comparing((Map<String, Object> e) -> e.get("invoiceType").toString())
                            .thenComparing(e -> ((String) e.get("totalMoney"))))
                    .peek(e -> e.put("sortNumber", counter.getAndIncrement()))
                    .collect(Collectors.toList());
            return ResultVO.success(sortedList);
        }

        // 有sortNumber则直接返回给前端进行排序
        return ResultVO.success(resultMapList);
    }
~~~
...
~~~
有一个List<String> invoiceNumberList，可能有重复发元素，如何快速获取到这个重复的元素
~~~
...
你可以使用 `Java 8` 的 `Stream` API 来快速找出 `List<String>` 中的重复元素：
...
public class Main {
    public static void main(String[] args) {
        List<String> invoiceNumberList = Arrays.asList("INV001", "INV002", "INV001", "INV003", "INV002");

        // 找出重复的元素
        Set<String> duplicates = invoiceNumberList.stream()
                .collect(Collectors.groupingBy(e -> e, Collectors.counting())) // 统计每个元素出现的次数
                .entrySet().stream()
                .filter(entry -> entry.getValue() > 1) // 过滤出出现次数大于1的元素
                .map(Map.Entry::getKey) // 只取键（元素值）
...
1. **`Collectors.groupingBy(e -> e, Collectors.counting())`**：按元素分组并统计出现次数。
2. **`.filter(entry -> entry.getValue() > 1)`**：筛选出出现次数大于1的元素。
3. **`.map(Map.Entry::getKey)`**：提取重复的元素。
4. **`.collect(Collectors.toSet())`**：将结果转换为 `Set`，避免重复。
...
* orElseThrow
  ...
  ~~~java
  InvoiceOrFolder rootFolder = rootFolderOptional.orElseThrow(() -> new IllegalArgumentException(String.format("发票文件夹不存在 folderId = %d", folderId)));
  ~~~
