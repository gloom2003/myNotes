...
- SpringMVC容器中的Controller需要依赖Service，写Controller时能从Spring容器中获取到所依赖的Service对象嘛？
...
- 是如何实现这样父子容器的关系的？
  ...
  简单来说，子容器中存储有父容器，父容器作为子容器类中的属性，当在子容器中找不到相应的对象时，就会去父容器中找。

  

- 是什么时候让两个容器产生这种父子容器的关系的？

  在ContextLoaderListener中，会在创建好容器后把容器存入**servletContext域**。这样在**DispatcherServlet启动时**，创建完SpringMVC容器后会从servletContext域中获取到Spring容器对象，设置Spring容器对象为SpringMVC容器的父容器,这样子容器就能获取到父容器中的bean了
...
~~~~xml
 <!--Spring-context-->
        ...
        <dependency>
            ...
            <!--配合tomcat插件时，如果不添加会报错:表示该依赖项在编译和测试阶段需要，但在运行阶段由容器或环境提供。-->
...
~~~~java
// 指定对象转换为JSON字符串的规则为：非nULL的字段才进行转换
...
​	注意，自己去使用AOP进行增强时，应该是对Service进行增强。不能对Controller进行增强，因为切面类会被放入父容器(Spring容器)，而我们的Controller是在子容器（MVC容器）中的。父容器中的切面类不能访问子容器的Controller方法来进行增强。
