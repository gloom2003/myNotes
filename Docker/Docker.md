...
### 2.3 docker run运行一个镜像

docker run hello-world  生成一个容器来运行这个 hello-world镜像

docker create hello-world 仅仅创建容器而不马上运行这个镜像

```sh
docker run --name=lbwnb hello-world   生成一个名为lbwnb的容器来运行这个 hello-world镜像
docker run --name=lbwnb --restart=always hello-world 表示停止运行后总是自动重启
```

部署nginx：

sudo docker run -d -p 80:80 nginx 在后台运行nginx并且进行端口映射

首先它会从镜像仓库中下载对应的镜像,实际上我们并没有在Ubuntu中安装Nginx，而是通过Docker运行下载好的镜像来进行服务器搭建的
...
先查看本地是否存在，查看缓存是否存在，都不存在时才去公共仓库进行下载

### 2.5  docker images 查看下载的所有docker镜像
...
### 2.6 docker ps查看当前容器的容器列表

docker ps -a  表示查看所有容器（其他选项可以使用-h查看）如果不加的话，只会显示当前**正在运行的容器**
...
### 2.8 docker rm 删除未在运行的容器

docker rm 容器id或名字

docker run --rm 镜像名称  表示容器在停止(docker stop)后将自动删除

### 2.9 docker rmi 删除未在运行的镜像
...
### 2.14 不会被删除退出容器快捷键 & docker attach回到指定id的正在运行的容器去
...
~~~sh
docker volume ls  表示查看由Docker进行管理的数据卷（挂载时没有指定宿主主机的文件时会生成）
docker volume inspect 数据卷名 表示查看(inspect)数据卷的详细信息
docker volume rm 数据卷名  表示删除数据卷
docker volume create lbwnb 表示创建一个由Docker进行管理的数据卷，命名为lbwnb
~~~
...
COPY target/DockerTest-0.0.1-SNAPSHOT.jar app.jar # 表示把target目录下面的DockerTest-0.0.1-SNAPSHOT.jar文件复制进服务器中docker的默认路径下面并命名为app.jar(相当于在 scp命令+mv命令?)
...
### 2.21 docker exec命令 在容器中启动一个新的终端

可以使用`exec`命令在容器中启动一个新的终端或是在容器中执行命令：

```sh
docker exec -it 容器名 bash  bash为一个解释程序也可以是需要执行的命令
```
...
会在容器中创建一个新的终端，把java --version这条命令的执行结果返回到当前的终端上，新创建的终端就结束了
~~~
...
**与stop+start命令不同的是**，stop命令会使整个进程结束，此时再使用start命令会重新开一个新的进程，而使用pause命令后，进程并没有结束，只是暂时被冻结了，使用uppause命令后进程将恢复正常状态

### 2.23 docker top 监控容器内部的进程信息
...
exit 退出base镜像，容器没有镜像在运行则会自动关闭

docker ps -a 查看所有的容器

docker start -i 容器id或名字  再次启动容器，不需要-t了

### 3.2使用Dockerfile方式打包镜像(命令方式)  docker build

也可以使用idea的DockerFile插件

1.创建并编写Dockerfile文件，写指令
...
2.执行docker build -t <构建的镜像名称> <构建到哪一个目录> 

```sh
docker build -t ununtu-java . 表示使用Dockerfile中的指令构建一个镜像到当前目录，并且命名为ununtu-java
```

对比commit方式的好处：

1. 每条指令执行之后，都会生成一个新的镜像层。所以使用docker history 来查看时会更加的详细 
2. 效率更高

### 3.3推送镜像到远程仓库

1.重新打标签(拷贝一份并且重命名),从而更加标准

docker tag ubuntu-java-file:latest 用户名/仓库名称:版本

```sh
docker tag ubuntu-java-file:latest kana669/ubuntu-java:1.0
表示拷贝一份ubuntu-java-file:latest文件并且重命名为kana669/ubuntu-java，修改版本为1.0
```
...
`-p`表示端口绑定，将Docker容器内的端口绑定到宿主机的端口上，这样就可以通过宿主的8080端口访问到容器的8080端口了
...
这个命令中，`-a` 参数用于显示所有的容器，包括运行中的和已经停止的容器；`-f` 参数用于筛选容器，并可以使用 `status=exited` 来筛选出状态为退出的容器。这样，你就可以只查看没有在运行的容器了。

```sh
docker rm $(docker ps -aq) 
```

### 3.6使用docker-compose部署一个简单的后端项目
...
services:   #services里面就是我们所有需要进行编排的服务了
  ...
  mysql:
  	...
  	environment:   #这里我们通过环境变量配置MySQL的root账号和密码
    	...
    	TZ: 'Asia/Shanghai'    #时区
  	ports:
  	- "3306:3306"    #把端口暴露出来，使外部可以访问，当然也可以不暴露，因为默认所有容器使用的是同一个网络,而不是外部访问内部
  redis:
...
```yaml
# 配置SpringBoot应用程序如何与数据库进行连接(在Docker中都使用的是同一个网络,不需要暴露端口，不需要NAT转换)
spring:
	datasource:
	...
	redis:
    	host: app_redis  # ip地址直接输入容器名称，会自动使用Docker提供的Dns服务器进行Dns解析
```
...
在本地使用sqlYou连接服务器的数据库，操作服务器的数据库(创建数据库、**创建用户(以后使用这个用户的账号和密码来登录包括springBoot程序中连接数据库时)**，授予权限(bugs:所有数据库的所有权限都给了springBoot程序才能正常连接数据库，讲师只给了需要的查询的数据库的使用权限),执行sql文件)
...
2. Docker服务端先看看**本地**有没有这个镜像，发现没有。
...
操作系统的镜像安装在电脑的硬盘上面运行，电脑的硬盘相当于操作系统的容器，

docker镜像也要在相应的容器上面运行,运行在启动镜像时创建的容器上，容器就是提供一个运行的环境,并且数据存储在容器中，容器被删除后数据就没有了，就好像电脑硬盘没了，数据也没了。有一些容器是一次性的：例如启动hello-world镜像时创建的hello-world容器，运行一次后就关闭了，想要查看需要使用docker ps -a命令来查看所有的容器。
...
​	下载的base镜像实际上只有CentOS的rootfs(用户空间的文件系统)，去除了bootfs（包括BootLoader和Linux内核）

​	base镜像底层会直接使用宿主主机的内核(bootfs有了)，也就是说你的Ubuntu内核版本是多少，那么base镜像中的CentOS内核版本就是多少，而rootfs则可以在不同的容器中运行多种不同的版本(所以使用不同的容器，不同的rootfs，相同的宿主主机的内核，**Docker就能够同时模拟多种Linux操作系统环境**)

​	(base镜像与宿主主机内核版本是一样的（这也是缺点所在，如果软件对内核版本有要求的话，那么此时使用Docker就直接寄了）)，

但是可以使用虚拟机安装对应版本的操作系统，再在上面使用docker即可
...
​	每安装一个软件，就在base镜像上一层层叠加上去，采用的是一种分层的结构,只需要在本地保存一份base镜像，就可以给多个不同的容器拼装使用.

​	最上层还有一个**可写容器层**:所有的镜像会叠起来组成一个统一的文件系统，如果不同层中存在相同位置的文件，那么上层的文件会覆盖掉下层的文件(因为从上往下读，找到第一个返回条件的就返回了)，最终我们看到的是一个叠加之后的文件系统。当我们需要修改容器中的文件时，实际上并不会对镜像进行直接修改，而是在最顶上的容器层（最上面一般称为容器层，下面都是镜像层(只读)）进行修改，不会影响到下面的镜像，否则镜像就很难实现多个容器共享了。
...
这样我们就可以直接在容器中访问宿主主机上的文件了，当然如果我们对**挂载**的容器目录中的文件进行编辑，那么相当于编辑的是宿主主机的数据。
...
那么就由Docker来自动创建一个目录(数据卷)（一个比较复杂的目录  /var/lib/... (存储了很多数据卷)并且需要root用户才能访问），并且**会将容器中对应路径下的内容拷贝到这个自动创建的目录中**，最后挂载到容器中，这种就是由Docker管理的数据卷了（docker managed volume）
...
**数据卷其实指向了/var/lib/目录下面的一个文件**,可以使用
...
表示使用自己创建的数据卷对容器的/root/test进行挂载并运行ubuntu-vim镜像，启动容器
~~~
...
限制为1后，只能最多使用一个CPU所能提供的资源，所以这里使用了两个CPU加在一起刚刚好为一个CPU所能提供的资源。
...
#### 1.使用在自己本地(idea)安装的DockerCompose
...
#### 2在Linux服务器上使用DockerCompose

基本命令：

~~~sh
sudo apt install docker-compoes # 下载docker-compoes
vim docker-compose.yml # 编写docker-compose.yml配置文件 其中的build需要使用image（Dockerfile文件中构建出来的镜像）替换，如上
docker-compoes up # 按照当前目录下的docker-compose.yml文件来部署环境
~~~

~~~sh
docker-compoes up -d # 后台运行
docker-compoes top #监控整套环境的状态
docker-compoes down # 先停止使用容器，然后删除使用容器，最后删除容器的网络
~~~
...
4.及时关闭Docker的外部访问
5.别用默认的端口，密码设置复杂些(Redis需要设置密码，可以使用某个文件的哈希值，保存备份好这个文件)，尽量设置限制IP访问
6.使用定时任务备份数据库
