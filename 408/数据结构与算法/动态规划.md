...
- 备忘录优化递归解法进行优化时间复杂度
- dp Table使用迭代的方式进行优化时间复杂度。进一步，可以使用空间压缩技巧优化空间复杂度

动态规划的关键在于「状态转移方程」，即如何通过若干个已计算出的「状态」推导出未计算的「状态」。

在这个推导「状态」的过程中，可能存在重复计算「状态」的情况（重叠子问题），需要备忘录技巧消除冗余计算。进一步，备忘录可以改造成 `dp` 数组，用非递归（迭代）的方式进行状态转移，也就是自底向上的动态规划解法。
    ...
    private static int dp(int n, int w, int[] values, int[] weights) {
        ...
        for(int i = 1;i<=n;i++){
            for(int j = 1;j<=w;j++){
                ...
                if(j < weights[i-1]){
                    // 背包装不下第i件物品时，dp[i][j]即为第i件物品前面所有物品存储到背包的最大价值，即dp[i-1][j]
                    dp[i][j] = dp[i-1][j];
                    continue;
                }
                // 取1与2的最大值
...
### 4.2 问能否能装满背包（或者最多装多少）

公式为：dp[j] = max(dp[j], dp[j - nums[i]] + nums[i]);
...
class Solution {
    public boolean canPartition(int[] nums) {
        ...
        // (2)选择：物品是否放入背包中
        // (3)dp数组含义：dp[i][j]表示前i件物品是否(true?false?)能够填满容器为j的背包中,题目所求即是dp[n][sum],sum已除2,所以数组初始化为new int[n+1][sum+1]
        sum /= 2;
        boolean[][] dp = new boolean[n+1][sum+1];
        // (4)base case:
        // 1.dp[0][j] = false
        // 2.dp[i][0] = true:剩下的容量为0表示已经被填满了。
        for(int i = 0;i<=n;i++){
            dp[i][0] = true;
        }
        for(int i = 1;i<=n;i++){
            for(int j = 1;j<=sum;j++){
                ...
                // (5) 状态转移方程：
                // 结果 = 选择：不放进背包是否能够填满与放进背包是否能够填满，只要一个能够填满即可证明dp[i][j]能够填满，所以使用 ||
...
解法1：代码随想录90% 一维dp Table求组合类问题,dp[j] += dp[j-nums[i]]
...
class Solution {
    public int findTargetSumWays(int[] nums, int target) {
        ...
        if(size < 0) size = -size;
        // 为什么dp Table初始化为size+1,而不是target+1
        // dp数组的定义：填满j（包括j）这么大容积的包，有dp[j]种方法
...
class Solution {
    public int change(int amount, int[] coins) {
        int n = coins.length;
        // dp数组的含义：dp[i][j] 前i个硬币凑成面额为j的组合数
        int[][] dp = new int[n+1][amount+1];
        // base case:
        // 凑成面额为j=0的方法有一种（什么也不做）,i=0个硬币什么面额也凑不出来
        for(int i = 0;i<=n;i++){
            dp[i][0] = 1;
        }
        // 动态规划框架：
        for(int i = 1;i<=n;i++){
            for(int j = 1;j<=amount;j++){
                if(j >= coins[i-1]){
                    // dp[i][j] = 硬币i不放背包的次数 + 硬币i放背包的次数
                    // 硬币i不放背包,所以前面dp[i-1],相当于禁用了i
                    // 硬币i放背包,由于每一种面额的硬币有无限个，所以dp[i],i还有并且有空间存放，为什么不存呢？
                    dp[i][j] = dp[i-1][j] + dp[i][j-coins[i-1]];
                }else{
                    // 没有空间存放这一个硬币i，硬币i不放背包,所以前面dp[i-1],相当于禁用了i
                    dp[i][j] = dp[i-1][j];
                }
            }
        }
