...
1. 设置两个头(http协议与浏览器厂商约定好的，你设置的响应头对了，响应的格式对了，浏览器就知道这是一个下载内容的响应，就会把响应流中的数据拿出来使用文件的形式供用户进行下载)
2. 把数据写入响应流当中
...
~~~~java
    ...
    public static void setDownLoadHeader(String filename, HttpServletResponse response) throws UnsupportedEncodingException {
        ...
        // 把文件名进行URL编码，后面浏览器会再进行解码，来解决乱码问题
...
~~~~java
    ...
    public void export(HttpServletResponse response){
        try {
            ...
            // 注意：此时响应体中全部是Excel文件数据，不能再写入JSON格式的数据了(return ResponseResult),
            // 否则区分不了响应体中是文件的二进制数据还是JSON格式的数据，会出问题，所以方法的返回值为void
        } catch (Exception e) {
            // 可能在写入的过程中出现异常，需要先重置响应体中的数据再写入新的数据
            response.reset();
            //出现异常则返回json数据
            e.printStackTrace();
